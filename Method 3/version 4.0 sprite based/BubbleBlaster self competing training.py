...
class Computer():
  ...
  def run_network(self, Bubble_list, opp_x_cord, opp_y_cord, opp_speed):
...
#Nice
  ...
  def move_computer(self, Bubble_list, opp_x_cord, opp_y_cord, opp_speed):
    output = self.run_network(Bubble_list, opp_x_cord, opp_y_cord, opp_speed)
...
class Match():
  total_games = sim_config["sim"]["MAX_GAMES"]
  sim_time = int(config["time"]["GAME_DURATION"] / sim_config["time"]["TIMESTEP"])
  def __init__(self, player_id, player, genome, net):
    self.Bubble_list = list()
    self.player_id = player_id
    self.player = player
    self.opponent = Computer(genome, net) 
  ...
  def move_all(self):
    ...
    self.player.move_computer(self.Bubble_list, self.opponent.x_cord, self.opponent.y_cord, self.opponent.speed)
    self.player.collision_and_score(self.Bubble_list)
    self.opponent.move_computer(self.Bubble_list, self.player.x_cord, self.player.y_cord, self.player.speed)
    self.opponent.collision_and_score(self.Bubble_list)
...
def Game(match):
  ...
  for _ in range(match.total_games):
    match.player.restart()
    match.opponent.restart()

    ###Main Program
    for __ in range(match.sim_time):    
      match.create_bubble()
      match.move_all()

    player_score = player_score + match.player.score

  return match.player_id, player_score / match.total_games

###Simulation controls the creation of matches and multiprocessing those matches
def Simulation(genomes, config_ai):
  ...
  match_list = [Match(id, player, opp_genome, opp_net) for id, player in enumerate(Player_list)]

  with concurrent.futures.ProcessPoolExecutor() as executor: #multiprocessing
    results = executor.map(Game, match_list)
    
    for result in results:
      player_id, player_score = result
      Player_list[player_id].genome.fitness = player_score
...
def ChooseRandom(genomes, config_ai):
  genome_list = list()
  net_list = list()
  for _, g in genomes:
    net = neat.nn.FeedForwardNetwork.create(g, config_ai)
    net_list.append(net)
    g.fitness = random.randint(0, 10000) / 100
    genome_list.append(g)
  
  for i in range(len(genome_list)-1, -1, -1):
    genome_list.pop(i)
    net_list.pop(i)
...
def run(config_path):
  ...
  rounds = int(input("Enter the amount of rounds the AI needs to train:"))

  if input("Type start_over if you want to retrain the AI:") == "start_over":
    print("Starting over...")

    if input("Do you want to generate a new ai_file.pickle (yes or no):") == "yes": #create new ai_file.pickle
      pop = neat.Population(config_ai)
      winner = pop.run(ChooseRandom, 1)
      with open(os.path.join("ai", "ai_file.pickle"), 'wb') as f:
        pickle.dump(winner, f)
      print("Created new dumb starting AI")

    pop = neat.Population(config_ai)
    starting_round = 0

  else:
    checkpoint_path = input("Enter the file path to the checkpoint file:")
    print("Continueing progress...")
    pop = neat.Checkpointer.restore_checkpoint(checkpoint_path)
    starting_round = int(input("What round number did you end:"))

  for round_num in range(starting_round, starting_round + rounds):
    print(f"New Round:{round_num}")
    ...
    pop.add_reporter(neat.Checkpointer(sim_config["sim"]["CHECKPOINT_STEP"], None, checkpoint_path))

    #Load opponent AI
    if round_num > 0:
      genome_path = os.path.join("ai", f"ai_file({round_num - 1}).pickle")
    else:
      genome_path = os.path.join("ai", f"ai_file.pickle")

    with open(genome_path, "rb") as f:
      global opp_genome, opp_net
      opp_genome = pickle.load(f)
      opp_net = neat.nn.FeedForwardNetwork.create(opp_genome, config_ai)

    #Run training process and save new opponent
    winner = pop.run(Simulation, sim_config["sim"]["GENERATIONS"])
    filename = os.path.join("ai", f"ai_file({round_num}).pickle")
    open(filename, "x") #creates new file
    with open(filename, 'wb') as f:
      pickle.dump(winner, f)
    
    #Load old checkpoint
    checkpoint_list = os.listdir("checkpoints")
    checkpoint_list.sort(key=lambda name : int(name[22:]))
    pop = neat.Checkpointer.restore_checkpoint(os.path.join("checkpoints", checkpoint_list[-1]))
    del checkpoint_list
