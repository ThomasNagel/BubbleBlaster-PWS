...
class Bubble():
  def __init__(self, is_speed_powerup):
    self.x_cord = config["field"]["x"] + config["bubble"]["GAP"] #x cord ligt een stukje buiten scherm
    self.y_cord = random.randint(0, config["field"]["y"]) #y cord is random hoogte in scherm
...
class Player():
  ...
  x_cord = config["field"]["x"] // 2
  y_cord = config["field"]["y"] // 2
...
class Computer(Player):
  ...
  def run_network(self, opp_x_cord, opp_y_cord, opp_speed):
    ...
    for i in range(len(temp_list) -1, -1, -1):
      if temp_list[i].x_cord < - sim_config["ai"]["SEE_GAP"] or temp_list[i].x_cord > config["field"]["x"] + sim_config["ai"]["SEE_GAP"]:
        del temp_list[i] 
    ...
    for i in range(len(d_list)):
      pygame.draw.line(screen, GREEN, (self.x_cord, self.y_cord), (d_list[i].x_cord, d_list[i].y_cord), 2)
  ...
  def move_computer(self, opp_x_cord, opp_y_cord, opp_speed):
    ...
    if self.x_cord < 0:
      self.x_cord = 0
    elif self.x_cord > config["field"]["x"]:
      self.x_cord = config["field"]["x"]
    #Bottom and top connected
    if self.y_cord < 0:
      self.y_cord = config["field"]["y"]
    elif self.y_cord > config["field"]["y"]:
      self.y_cord = 0
    #Speed decrease 
