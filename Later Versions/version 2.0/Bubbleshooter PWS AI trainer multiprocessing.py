...
class Match():
  def __init__(self, play_one, play_two, play_list):
    self.Bubble_list = list()
    self.play_one = play_one
    self.play_two = play_two
  ...
  def move_all(self):
    ...
    self.play_list[self.play_one].move_computer(self.Bubble_list, self.play_list[self.play_two].x_cord, self.play_list[self.play_two].y_cord, self.play_list[self.play_two].speed)
    self.play_list[self.play_one].collision_and_score(self.Bubble_list)
    self.play_list[self.play_two].move_computer(self.Bubble_list, self.play_list[self.play_one].x_cord, self.play_list[self.play_one].y_cord, self.play_list[self.play_one].speed)
    self.play_list[self.play_two].collision_and_score(self.Bubble_list)

###Functions   

def Game(match):
  match.play_list[match.play_one].score = 0
  match.play_list[match.play_two].score = 0
  ...
  return (match.play_one, match.play_list[match.play_one].score, match.play_two, match.play_list[match.play_two].score)
...
def Simulation(genomes, config_ai):
  ...
  for i in range(1, sim_config["sim"]["MAX_GAMES"] + 1):
    """Matchmaking: elke ai (op even positie) speelt een game tegen MAX_GAMES dichtsbijzijnde ais (op oneven positie)"""
    match_list = []
    for j in range(len(Player_list)):
      if j % 2 != 0:
        opp = (j + (2*i) - 1) % len(Player_list) 
        match_list.append(Match(j, opp, Player_list))
    with concurrent.futures.ProcessPoolExecutor() as executor: #multiprocessing
      results = executor.map(Game, match_list)
      
      for result in results:
        play_one, score_one, play_two, score_two = result
        Player_list[play_one].genome.fitness = Player_list[play_one].genome.fitness + score_one / sim_config["sim"]["MAX_GAMES"]
        Player_list[play_two].genome.fitness = Player_list[play_two].genome.fitness + score_two / sim_config["sim"]["MAX_GAMES"]
...
def run(config_path):
  ...
  if input("Type start_over if you want to retrain the AI:") == "start_over":
    ...
    time.sleep(0.5)
    pop = neat.Checkpointer.restore_checkpoint("neat_checkpoint.pickle3208")
